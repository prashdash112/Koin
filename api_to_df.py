...
def extractor(smb,mkt):
    ...
    '''
    global currency
    currency = mkt        #changes 
    url = "https://alpha-vantage.p.rapidapi.com/query"
    querystring = {"function":"DIGITAL_CURRENCY_DAILY","market": mkt,"symbol":smb}
    headers = {
    ...
    global api_request
    api_request = response['Time Series (Digital Currency Daily)'] #changes
    return api_request
...
def api_df(response):
    ...
    for i in response.keys():
        ...
        open_tck.append(response[i]['1a. open ({k})'.format(k = currency)])
        high.append(response[i]['2a. high ({k})'.format(k = currency)])
        low.append(response[i]['3a. low ({k})'.format(k = currency)])
        close.append(response[i]['4a. close ({k})'.format(k = currency)])
